...
/**
 * Sube al completo el estado del localStorage a Supabase.
 * Se llama una sola vez cuando un usuario existente hace login y la BD está vacía.
 */
export async function migrateLocalStateToDb(userId, state) {
  console.log('[db] Migrando estado local → Supabase…');
  ...
  if (state.habits?.length) {
    await saveHabits(userId, state.habits);
  }

  // Historial — aplanar el objeto { date: { habitId: status } } → rows
  const historyEntries = [];
  for (const [date, dayMap] of Object.entries(state.history ?? {})) {
    for (const [habitId, status] of Object.entries(dayMap)) {
      historyEntries.push({ habitId, date, status });
    }
  }
  if (historyEntries.length) {
    await saveHabitEntries(userId, historyEntries);
  }
  ...
  if (state.activeEffects?.length) {
    await saveActiveEffects(userId, state.activeEffects);
  }

  // Logros
  if (state.unlockedAchievements?.length) {
    await saveAchievements(userId, state.unlockedAchievements);
  }

  // Reto diario
  if (state.currentDaily && state.lastDailyDate) {
    await saveDaily(userId, state.currentDaily, state.lastDailyDate);
  }
